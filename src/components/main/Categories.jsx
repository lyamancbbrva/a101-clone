  ...
  const id = cat
    .find(
      (item) =>
        ...
        catName?.toLocaleLowerCase()
    )
    ?.subcategory.find(
      (elem) => elem.name.replace(/[\s,&]/g, "").toLowerCase() == subcatName
    ).id;
    ...
    <div className="bg-[#F3F6FA]">
      ...
      >
        <div
          className={`scroll ${
            hidden ? "" : "hidden"
          } max-w-[100px]  md:max-w-[150px] max-h-[100vh] overflow-y-scroll`}
        >
          {mainCategory &&
        ...
        <div className={`w-full  scroll overflow-y-scroll max-h-[100vh]`}>
          ...
          <ul
            className={`${
              productHidden ? "hidden" : "inline-block"
            } w-full min-h-[80vh]`}
          >
            <li
              onClick={() => setHidden(false)}
            ...
            {cat &&
              cat
                ?.find((elem) => elem.id == catId)
                ?.subcategory?.map((item, index) => (
                  <Link to={`/${item.slug}`}>
                  <li
                    onClick={getProductsBySubcat}
                    key={index}
                    className="flex cursor-pointer justify-between text-[#333] border-b py-4 w-full text-sm px-3 items-center"
                  >
                    {item.name}
      ...
      <div className="mobilCat block lg:hidden">
        ...
        <div
          className={`wrapper px-3 bg-[#F3F6FA] ${
            productHidden ? "block" : "hidden"
          }`}
        >
          <h2 className="py-4">
            "{subCategory}" için
          ...
          <div className="min-w-[15vw]">
            {
              ...
              <div className=" mb-6">
                ...
                <div className="flex justify-between ">
                  <span class="text-sm  text-[#333]  start-0 -bottom-6">
                    t500
                  </span>
                  <span class="text-sm  text-[#333]  end-0 -bottom-6">
                    t{!inp ? '5000' : inp}
                  </span>
                </div>
                </div>
            ...
            <div>
             {
              ...
              <div
                ...
                } absolute max-h-[40vh] min-w-[20vw] overflow-auto bg-white py-3 mb-3 px-4 rounded-xl text-md shadow z-50`}
              >
                <ul>
                  <Link className="text-[#00BAD3]">
                  ...
                  <Link>
                    <li
                      ...
                      className="text-[.95em] py-2 "
                    >
                      Fiyatl Yüksekten Düşüğe
                    </li>
                  </Link>
                  <Link>
                    <li   onClick={() =>
