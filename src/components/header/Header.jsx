            ...
            <nav>
                ...
                <div className='icons flex lg:hidden justify-between px-3 fixed w-full bottom-0 bg-white pt-2.5'>
                    ...
                    <Link to={'/sepet'} onClick={() => setActiveTab(3)}  className='icon relative flex flex-col items-center'>
                        ...
                        <div className={`bg-[#00BAD3] ${items.length > 0 ? 'block' : 'hidden'} rounded-full w-[12px] h-[12px] text-white text-[.5em] px-1  absolute right-0 `}>{items.length}</div>
                        <h5 className={`${activeTab == 3 ? 'text-[#00BAD3]' : items.length == 0 ? 'text-[#8D939C]' : '' }   ${items.length > 0 ? 'w-[40px] px-2 rounded-2xl text-white bg-[#00BAD3] text-center text-[.6em] h-[15px] my-1' :  'py-1 text-[.5em]'} `}>
                            {items.length > 0 ? 't' + items.map(item => +item.price).reduce((sum, item) => sum + item, 0) : 'Sepetim'}
                        </h5>
                    </Link>
